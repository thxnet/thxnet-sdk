...
#                                 Identity v1→v2 MBM stays at v1 until drained but
#                                 --disable-mbm-checks handles that.
...
run_try_runtime() {
    ...
    if [[ ${exit_code} -eq 0 ]]; then
        ...
        # Identity v1→v2 is a multi-block migration (SteppedMigration) that stays at
        # storage version v1 until drained across multiple blocks. try-runtime can't
