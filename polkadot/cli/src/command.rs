...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			...
			"thxnet-dev" => Box::new(polkadot_service::chain_spec::thxnet_development_config()?),
			#[cfg(feature = "thxnet-native")]
			"thxnet-local" => Box::new(polkadot_service::chain_spec::thxnet_local_testnet_config()?),
			...
			path => {
					...
					chain_spec.is_versi()
				{
					...
					Box::new(polkadot_service::ThxnetChainSpec::from_json_file(path)?)
				} else {
					chain_spec
				}
			},
