...
//! - **Reputation Registry**: Feedback authorization, submission, and management
...
pub mod pallet {
	...
	pub struct EscrowInfo<T: Config> {
		...
		pub auto_complete_at: BlockNumberFor<T>,
		/// Block number after which the client can cancel (if not
		/// auto-completed)
		pub timeout: BlockNumberFor<T>,
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		type ValidationDeadline: Get<BlockNumberFor<Self>>;

		/// Escrow auto-complete duration (number of blocks, e.g., 7 days)
		#[pallet::constant]
		type EscrowAutoCompleteBlocks: Get<BlockNumberFor<Self>>;

		/// Feedback rate limit duration (number of blocks, e.g., 7 days)
		#[pallet::constant]
		type FeedbackRateLimitBlocks: Get<BlockNumberFor<Self>>;
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// # Parameters
		/// - `registration_uri`: URI pointing to registration file (IPFS hash or HTTP URL)
		...
		pub fn authorize_feedback(
			...
			client: T::AccountId,
			index_limit: u32,
			expiry_blocks: BlockNumberFor<T>,
		) -> DispatchResult {
			let who = ensure_signed(origin)?;
		...
		/// - `ValidatorNotWhitelisted`: Validator not in whitelist (if whitelist is enabled)
		#[pallet::call_index(6)]
		...
		/// # Errors
		/// - `AgentNotFound`: Agent does not exist
		/// - `TooManyValidationRequests`: Too many validation requests for this agent
		#[pallet::call_index(8)]
		...
		/// - `ValidationAlreadySubmitted`: Already submitted validation for this request
		/// - `InvalidScore`: Score not in range 0-100
		#[pallet::call_index(9)]
		...
		/// - `custom_auto_complete_blocks`: Optional custom auto-complete duration. If None, uses
		///   `EscrowAutoCompleteBlocks`.
		...
		pub fn create_escrow(
			...
			amount: BalanceOf<T>,
			timeout: BlockNumberFor<T>,
			custom_auto_complete_blocks: Option<BlockNumberFor<T>>,
		) -> DispatchResult {
			let who = ensure_signed(origin)?;
		...
		/// - `CannotDisputeEscrow`: Escrow is not in Active or AutoCompleted status
		...
		/// - `favor_client`: If true, refund to client; if false, allow agent to claim
		///
		/// # Errors
		/// - `EscrowNotFound`: Escrow does not exist
