...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		type MinCampaignDuration: Get<BlockNumberFor<Self>>;
		#[pallet::constant]
		type MaxCampaignDuration: Get<BlockNumberFor<Self>>;
		#[pallet::constant]
		...
		type LicenseVerifier: LicenseVerifier<Self::AccountId, BlockNumberFor<Self>>;
	...
	pub enum Event<T: Config> {
		#[codec(index = 0)]
		CampaignCreated { campaign_id: u32, creator: T::AccountId, deadline: BlockNumberFor<T> },
		#[codec(index = 1)]
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_campaign(
			...
			CreatorCampaigns::<T>::try_mutate(&creator, |ids| -> DispatchResult {
				ids.try_push(campaign_id)
					.map_err(|_| Error::<T>::MaxCampaignsPerCreatorReached)?;
				Ok(())
			})?;
		...
		pub fn claim_funds(origin: OriginFor<T>, campaign_id: u32) -> DispatchResult {
			...
			// 2. The campaign already met its goal and was finalized to Succeeded — the economic
			//    outcome is decided.
			// 3. `report_license_revoked` protects investors during Funding / Paused /
			//    MilestonePhase.  Once Succeeded, the creator has earned the right to claim.
			// 4. The V3 concern ("transferred participation blocks original creator") conflated RWA
			//    payer with campaign creator — these are independent concepts.
	...
	impl<T: Config> Pallet<T> {
		...
		fn do_transfer(
			...
			match currency {
				...
				PaymentCurrency::Asset(asset_id) => {
					...
					let preservation = if matches!(existence_req, ExistenceRequirement::KeepAlive) {
		...
		/// Check investor eligibility for a campaign.
		pub fn check_eligibility(
			campaign_id: u32,
			who: T::AccountId,
		) -> Result<(), EligibilityError> {
			let campaign =
				Campaigns::<T>::get(campaign_id).ok_or(EligibilityError::CampaignNotFound)?;
			Self::check_eligibility_inner(&who, &campaign, campaign_id).map_err(|_| {
				// Map the DispatchError to the appropriate EligibilityError.
				// The inner function already checks each rule, but we need
				// to map back. For simplicity, iterate and check.
				for rule in campaign.eligibility_rules.iter() {
					let passes = match rule {
						EligibilityRule::NativeBalance { min_balance } =>
							T::NativeCurrency::free_balance(&who) >= *min_balance,
						...
						EligibilityRule::NftOwnership { required_sets } =>
							required_sets.iter().all(|set| {
								set.iter().any(|(cid, iid)| {
									<T::NftInspect as nonfungibles_v2::Inspect<T::AccountId>>::owner(cid, iid)
                                            .map_or(false, |owner| owner == who)
								})
							}),
						EligibilityRule::AccountWhitelist =>
							CampaignWhitelist::<T>::contains_key(campaign_id, &who),
					};
					if !passes {
						return match rule {
							EligibilityRule::NativeBalance { .. } =>
								EligibilityError::InsufficientNativeBalance,
							EligibilityRule::AssetBalance { .. } =>
								EligibilityError::InsufficientAssetBalance,
							EligibilityRule::NftOwnership { .. } =>
								EligibilityError::NftOwnershipNotMet,
							EligibilityRule::AccountWhitelist => EligibilityError::NotWhitelisted,
						};
					}
				}
				// Fallback (should not reach here if check_eligibility_inner failed).
				EligibilityError::CampaignNotFound
			})
		}
		...
		pub fn preview_withdrawal(
			...
			investor: T::AccountId,
			amount: BalanceOf<T>,
		) -> Option<WithdrawalPreview<BalanceOf<T>>> {
			let campaign = Campaigns::<T>::get(campaign_id)?;
			let investment = Investments::<T>::get(campaign_id, &investor)?;
			let remaining = investment.total_invested.saturating_sub(investment.total_withdrawn);
			if amount > remaining || amount.is_zero() {
				return None;
			}

			let penalty_bps = match campaign.status {
				CampaignStatus::Funding => campaign
					.config
					.early_withdrawal_penalty_bps
					.unwrap_or(T::EarlyWithdrawalPenaltyBps::get()),
				_ => 0,
			};

			let penalty = if penalty_bps > 0 {
				// Ceiling division: (amount * bps + 9999) / 10000
				let numerator = amount.saturating_mul(penalty_bps.into());
				numerator.saturating_add(9999u32.into()) / 10000u32.into()
			} else {
				Zero::zero()
			};
			let net_amount = amount.saturating_sub(penalty);

			Some(WithdrawalPreview { gross_amount: amount, penalty, net_amount, penalty_bps })
		}

		/// Build a summary of a campaign for read-only queries.
		pub fn campaign_summary(
			campaign_id: u32,
		) -> Option<CampaignSummary<BalanceOf<T>, BlockNumberFor<T>>> {
			let c = Campaigns::<T>::get(campaign_id)?;
			let now = frame_system::Pallet::<T>::block_number();

			let goal = match &c.config.funding_model {
				FundingModel::AllOrNothing { goal } => Some(goal.clone()),
				FundingModel::MilestoneBased { goal, .. } => Some(goal.clone()),
				FundingModel::KeepWhatYouRaise { .. } => None,
			};

			let remaining_blocks = if c.config.deadline > now {
				Some(c.config.deadline.saturating_sub(now))
			} else {
				None
			};

			let funding_percentage_ppm = if let Some(ref g) = goal {
				if !g.is_zero() {
					Permill::from_rational(c.total_raised, *g).deconstruct()
				} else {
					0
				}
			} else {
			...
			let (milestones_completed, milestones_total) = match &c.config.funding_model {
				FundingModel::MilestoneBased { milestones, .. } => {
		...
		/// List all campaign IDs created by `creator`.
		pub fn campaigns_by_creator(creator: T::AccountId) -> alloc::vec::Vec<u32> {
			CreatorCampaigns::<T>::get(&creator).into_inner()
		}

		/// List all campaign IDs invested in by `investor`.
		pub fn campaigns_by_investor(investor: T::AccountId) -> alloc::vec::Vec<u32> {
			InvestorCampaigns::<T>::get(&investor).into_inner()
		}

		/// Get a specific investment record.
		pub fn get_investment(
			campaign_id: u32,
			investor: T::AccountId,
		) -> Option<Investment<BalanceOf<T>>> {
			Investments::<T>::get(campaign_id, &investor)
		}

		/// Get current protocol config (fee bps + recipient).
		pub fn get_protocol_config() -> (u16, T::AccountId) {
			let bps =
				ProtocolFeeBpsOverride::<T>::get().unwrap_or_else(|| T::ProtocolFeeBps::get());
			let recipient = ProtocolFeeRecipientOverride::<T>::get()
				.unwrap_or_else(|| T::ProtocolFeeRecipient::get());
			(bps, recipient)
		}
	}
}
