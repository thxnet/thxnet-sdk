...
pub mod pallet {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn try_state(_n: BlockNumberFor<T>) -> Result<(), sp_runtime::TryRuntimeError> { Self::do_try_state() }
    }
    ...
    impl<T: Config> Pallet<T> {
            ...
            match currency {
                ...
                PaymentCurrency::Asset(asset_id) => {
                    ...
                    <T::Fungibles as fungibles::Mutate<T::AccountId>>::transfer(
                        *asset_id, from, to, amount, preservation,
                    )?;
                }
            }
            Ok(())
        ...
        pub fn check_eligibility(
            ...
            let campaign = Campaigns::<T>::get(campaign_id)
                .ok_or(EligibilityError::CampaignNotFound)?;
            Self::check_eligibility_inner(&who, &campaign, campaign_id)
                .map_err(|_| {
                    ...
                    for rule in campaign.eligibility_rules.iter() {
                        ...
                        if !passes {
                            return match rule {
                                ...
                                EligibilityRule::AccountWhitelist =>
                                    EligibilityError::NotWhitelisted,
                            };
                        }
                    }
        ...
        pub fn preview_withdrawal(
            ...
            let penalty_bps = match campaign.status {
                CampaignStatus::Funding => {
                    campaign.config.early_withdrawal_penalty_bps.unwrap_or(
                        T::EarlyWithdrawalPenaltyBps::get()
                    )
                },
                _ => 0,
            };
        ...
        pub fn get_protocol_config() -> (u16, T::AccountId) {
            let bps = ProtocolFeeBpsOverride::<T>::get()
                .unwrap_or_else(|| T::ProtocolFeeBps::get());
