	...
	AssetId: Codec + Send + Sync + 'static,
{
	fn effective_participation_status(
		...
		api.effective_participation_status(at_hash, asset_id, participation_id)
			.map_err(|e| {
				jsonrpsee::types::ErrorObjectOwned::owned(-1, format!("{:?}", e), None::<()>)
			})
	}
	...
	fn can_participate(
		...
		let api = self.client.runtime_api();
		let at_hash = at.unwrap_or_else(|| self.client.info().best_hash);
		api.can_participate(at_hash, asset_id, who).map_err(|e| {
			jsonrpsee::types::ErrorObjectOwned::owned(-1, format!("{:?}", e), None::<()>)
		})
	}
	...
	fn assets_by_owner(
		...
		let api = self.client.runtime_api();
		let at_hash = at.unwrap_or_else(|| self.client.info().best_hash);
		api.assets_by_owner(at_hash, owner).map_err(|e| {
			jsonrpsee::types::ErrorObjectOwned::owned(-1, format!("{:?}", e), None::<()>)
		})
	}
	...
	fn participations_by_holder(
		...
		let api = self.client.runtime_api();
		let at_hash = at.unwrap_or_else(|| self.client.info().best_hash);
		api.participations_by_holder(at_hash, holder).map_err(|e| {
			jsonrpsee::types::ErrorObjectOwned::owned(-1, format!("{:?}", e), None::<()>)
		})
	}
	...
	fn active_participant_count(
		...
		let api = self.client.runtime_api();
		let at_hash = at.unwrap_or_else(|| self.client.info().best_hash);
		api.active_participant_count(at_hash, asset_id).map_err(|e| {
			jsonrpsee::types::ErrorObjectOwned::owned(-1, format!("{:?}", e), None::<()>)
		})
	}
}
