...
//!   - **Attack Succeeds**: demonstrates that the vulnerability exists in the current code (the
//!     test PASSES, proving the bug is present).
//!   - **Attack Defended**: demonstrates that the pallet correctly rejects the adversarial action
//!     (the test PASSES, proving the defense works).
//!
...
mod cat2_state_machine {
	...
	// 2. Payer calls exit_participation — lazy settlement fires, refunding deposit.
	//    exit_participation returns Ok(()) via early return.
	// 3. Payer calls exit_participation AGAIN — if lazy settlement doesn't gate properly, another
	//    refund could occur.
	//
	...
	// 2. Owner deactivates asset (participants can't request new, but existing remain active).
	// 3. Owner sunsets asset.
	// 4. Asset auto-retires via on_initialize.
	// 5. Participants are trapped — their status is still Active but the asset is Retired. They
	//    must call claim_retired_deposit to recover.
	//
...
mod cat5_7_storage_and_bounded {
	...
	// 2. For each asset, attacker calls request_participation listing the VICTIM as a holder
	//    (blanket ParticipationFilter allows this).
	// 3. After MaxParticipationsPerHolder (5) participations, the victim cannot participate in ANY
	//    new asset.
	//
	...
	fn attack_cat5_3_push_holder_asset_silent_failure_mitigated() {
		...
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				let new_aid = register_test_asset(BOB, ALICE, default_policy());
				assert_noop!(
					Rwa::request_participation(RuntimeOrigin::signed(DAVE), new_aid, vec![CHARLIE],),
					Error::<Test>::MaxParticipationsPerHolderReached
				);
...
mod cat7_batch_reject {
	...
	// 4. If one refund fails (e.g., pallet account drained), the ? operator propagates the error.
	// 5. QUESTION: Does the PendingApprovals::take get rolled back? In FRAME v4, dispatchables are
	//    transactional by default, so YES.
	//
...
mod cat8_toctou {
	...
	// 3. renew_participation is called — should work from Expired state, re-collecting the deposit.
