...
/// # Version History
///
/// - **V1–V4**: Implicit fixes and feature additions applied during early development.  No on-chain
///   `StorageVersion` marker existed at those stages, so there are no migration modules for these
///   versions.
/// - **V5** (current): Added `MinParticipationDeposit` policy enforcement. This is the first
///   version tracked by an explicit `StorageVersion`. No schema change — the storage layout itself
///   is unchanged; only the extrinsic validation logic was tightened.
///
/// # How to add a future migration (e.g. V6)
///
/// 1. Create a new sub-module `pub mod v6 { ... }` below.
/// 2. Inside the module, define any `OldFoo` structs that capture the **pre-migration** layout
///    (copy the current struct and rename it).
/// 3. Implement `OnRuntimeUpgrade` for `MigrateToV6<T>`:
///    - In `on_runtime_upgrade()`: a. Guard: if on-chain version != 5, skip. b. Translate / mutate
///      storage as needed. c. Call `StorageVersion::new(6).put::<pallet::Pallet<T>>()`. d. Return
///      the consumed weight.
///    - In `pre_upgrade()` (behind `try-runtime`): snapshot any counters needed for post-upgrade
///      assertions.
///    - In `post_upgrade()` (behind `try-runtime`): assert invariants.
/// 4. Wire the migration into the runtime's `Executive` type or `type Migrations` tuple so it runs
///    on the next upgrade.
