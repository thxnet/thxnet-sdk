...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		#[codec(index = 4)]
		AssetSunsetting { asset_id: u32, expiry_block: BlockNumberFor<T> },
		#[codec(index = 5)]
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_asset(
			...
			policy: AssetPolicy<BalanceOf<T>, BlockNumberFor<T>, T::AssetId>,
			metadata: Vec<u8>,
		) -> DispatchResult {
		...
		pub fn update_asset_policy(
			origin: OriginFor<T>,
			rwa_asset_id: u32,
			new_policy: AssetPolicy<BalanceOf<T>, BlockNumberFor<T>, T::AssetId>,
		) -> DispatchResult {
	...
	impl<T: Config> Pallet<T> {
		...
		fn do_try_state() -> Result<(), sp_runtime::TryRuntimeError> {
			...
			for (owner, asset_ids) in OwnerAssets::<T>::iter() {
				for &aid in asset_ids.iter() {
					let asset =
						RwaAssets::<T>::get(aid).ok_or(sp_runtime::TryRuntimeError::Other(
							"OwnerAssets references non-existent asset",
						))?;
			...
			for (asset_id, account, pid) in HolderIndex::<T>::iter() {
				let p = Participations::<T>::get(asset_id, pid).ok_or(
					sp_runtime::TryRuntimeError::Other(
						"HolderIndex references non-existent participation",
					),
				)?;
				if !matches!(
					p.status,
			...
			for (asset_id, pending_ids) in PendingApprovals::<T>::iter() {
				for &pid in pending_ids.iter() {
					let p = Participations::<T>::get(asset_id, pid).ok_or(
						sp_runtime::TryRuntimeError::Other(
							"PendingApprovals references non-existent participation",
						),
					)?;
					if !matches!(p.status, ParticipationStatus::PendingApproval) {
			...
			for (block, asset_ids) in SunsettingAssets::<T>::iter() {
				for &aid in asset_ids.iter() {
					let asset =
						RwaAssets::<T>::get(aid).ok_or(sp_runtime::TryRuntimeError::Other(
							"SunsettingAssets references non-existent asset",
						))?;
					match asset.status {
			...
			for (asset_id, pending_owner) in PendingOwnershipTransfer::<T>::iter() {
				let asset =
					RwaAssets::<T>::get(asset_id).ok_or(sp_runtime::TryRuntimeError::Other(
		...
		/// Returns the effective participation status, settling lazy expiry
		/// if applicable. Read-only (does NOT write to storage).
		pub fn effective_participation_status(
			asset_id: u32,
			participation_id: u32,
		) -> Option<ParticipationStatus<BlockNumberFor<T>>> {
			let mut p = Participations::<T>::get(asset_id, participation_id)?;
			// Check lazy expiry without writing to storage.
			if let ParticipationStatus::Active { expires_at: Some(expiry), .. } = p.status {
				let now = frame_system::Pallet::<T>::block_number();
				if now >= expiry {
					p.status = ParticipationStatus::Expired;
		...
		/// Check whether `who` can participate in the given asset.
		pub fn can_participate(
			asset_id: u32,
			who: T::AccountId,
		) -> Result<(), CanParticipateError> {
			let asset = RwaAssets::<T>::get(asset_id).ok_or(CanParticipateError::AssetNotFound)?;
			if !matches!(asset.status, AssetStatus::Active) {
				return Err(CanParticipateError::AssetNotActive);
			}
			if let Some(max) = asset.policy.max_participants {
				if asset.participant_count >= max {
					return Err(CanParticipateError::MaxParticipantsReached);
				}
			}
			// Check if already participating via HolderIndex.
			if HolderIndex::<T>::contains_key(asset_id, &who) {
				return Err(CanParticipateError::AlreadyParticipating);
			}
			// Check participation filter (KYC/whitelist).
			T::ParticipationFilter::ensure_eligible(asset_id, &who)
				.map_err(|_| CanParticipateError::NotEligible)?;
			Ok(())
		}

		/// List all asset IDs owned by `owner`.
		pub fn assets_by_owner(owner: T::AccountId) -> alloc::vec::Vec<u32> {
			OwnerAssets::<T>::get(&owner).into_inner()
		}

		/// List all (asset_id, participation_id) pairs for `holder`.
		pub fn participations_by_holder(holder: T::AccountId) -> alloc::vec::Vec<(u32, u32)> {
			let asset_ids = HolderAssets::<T>::get(&holder);
			let mut result = alloc::vec::Vec::new();
			for &aid in asset_ids.iter() {
				if let Some(pid) = HolderIndex::<T>::get(aid, &holder) {
					result.push((aid, pid));
				}
			}
			result
		...
		/// Count active participants for the given asset.
		pub fn active_participant_count(asset_id: u32) -> u32 {
			RwaAssets::<T>::get(asset_id).map(|a| a.participant_count).unwrap_or(0)
		}
	}
}
